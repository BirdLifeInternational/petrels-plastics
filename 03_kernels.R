...
#Minor edits Beth Clark Mar 2020
#Adapted without age classes, but with months Beth Clark Mar 2020 
...
.libPaths() <- c(.libPaths()[1],
                    "C:/Users/bethany.clark/Documents/R/R-3.6.2/library")
...
#dir.create(paste0(dir,"/scripts_results/03_kernels")) #added - BC
#dir.create(paste0(dir,"/scripts_results/03_kernels/locations_plots")) #added - BC
#dir.create(paste0(dir,"/scripts_results/03_kernels/kde_plots")) #added - BC
dir_kernels <- paste0(dir,"/scripts_results/03_kernels") #changed - BC
...
dataset_number <- 13
#c(20,21,74,127,18,123,32,96,124,23)

#8 failed
#94  "Ardenna grisea_Codfish Island.csv"

for(dataset_number in 13){ #
  ...
  for (month_number in 1:length(months)){ #1:length(months)
    ...
    if(nrow(tracks_wgs) > 4){
      ...
      #one dataset had too big a range, so a smnall amount was cropped ####
