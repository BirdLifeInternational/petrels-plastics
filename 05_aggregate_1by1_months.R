...
#dir.create(paste0(dir,"/scripts_results/04_aggregate_1by1_grid/")) 
#dir.create(paste0(dir,"/scripts_results/04_aggregate_1by1_grid/maps/")) #added - BC
#dir.create(paste0(dir,"/scripts_results/04_aggregate_1by1_grid/na_maps/")) #added - BC
...
redblus <- c("#f0faff","#a60000")

cols <- colorRampPalette(redblus)(30)
...
for (i in 1:length(files)){#
  ...
  c <- sum(getValues(a_proj2)) # total number of birds in the 1 x 1 degree raster
  ...
  plot(over,main=paste0("pieces per km2 * seabird = ",over_val),
  ...
  par(mfrow=c(1,2))
  plot(a_proj2,main=name,col=colsviri,legend=F)
  plot(na_over,main=paste0("pieces per km2 * seabird Nas = ",sum(na_over@data@values)),
       col=colsviri,legend=F)
  dev.off()
  }
