...
for (i in 1:length(species)){
  ...
  species_weights$seasons[i] <- max(sp_weightings$seasons)
  raster_name <- paste0(dir,"/outputs/06_species/",species[i],".tif")
...
for (i in 1:length(species)){
  sp <- raster(paste0(dir,"/outputs/06_species/",species[i],".tif"))
...
raster_name_2 <- paste0(dir,"/outputs/all_birds_distribution.tif")
