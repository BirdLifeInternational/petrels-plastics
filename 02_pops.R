...
################# LOADING SPP DATA ##################
data_std <- paste0(dir, "/scripts_results/01_cleaning_data/")
...
write.csv(files_list,paste0(dir,"/scripts_results/02_pops.csv"),row.names = F)
...
all_pops2 <- all_pops[!is.na(all_pops$scientific_name), ]
write.csv(all_pops,paste0(dir,"/scripts_results/all_locations2.csv"), row.names = F)

all_pops3 <- read.csv(paste0(dir,"/scripts_results/all_locations.csv"))

plot(all_pops$longitude,all_pops$latitude)
points(all_pops3$longitude,all_pops3$latitude,col="red")
plot(all_pops$longitude,all_pops$latitude,col="black")

head(all_pops)
head(all_pops3)

all_pops$id <- paste(all_pops$dataset_id,all_pops$track_id,
                     all_pops$latitude,all_pops$longitude,all_pops$dtime,sep="_")
all_pops3$id <- paste(all_pops3$dataset_id,all_pops3$track_id,
                     all_pops3$latitude,all_pops3$longitude,all_pops3$dtime,sep="_")

all_pops$match <- ifelse(all_pops$id %in% all_pops3$id,"yes","no")
table(all_pops$match)

yes <- all_pops[all_pops$match == "yes",]

plot(yes$longitude,yes$latitude)

all_pops4 <- all_pops[duplicated(all_pops)]
